            ...
            foreach (var variableList in variableLists)
            {
                foreach (var fsmVariable in variableList.GetAllNamedVariables())
                {
                    ...
                    if (fsmVariable.GetType() == typeof(FsmArray))
                        arrayDict.Add(fsmVariable.Name, ((FsmArray)fsmVariable).Values);
                    else
                        dict.Add(fsmVariable.Name, fsmVariable.RawValue);
                }
            }

            ES3.Save<PMDataWrapper>(key.Value, new PMDataWrapper(dict, arrayDict), GetSettings());
        }
    }
...
#region Load Actions
    ...
    public class LoadAll : SettingsAction
    {
        ...
        public override void Enter()
        {
            ...
            var wrapper = ES3.Load<PMDataWrapper>(key.Value, GetSettings());
            ...
            foreach (var variableList in variableLists)
            {
                foreach (var fsmVariable in variableList.GetAllNamedVariables())
                {
                    if (fsmVariable.GetType() == typeof(FsmArray))
                    {
                        if (wrapper.arrays.ContainsKey(fsmVariable.Name))
                            ((FsmArray)fsmVariable).Values = wrapper.arrays[fsmVariable.Name];
                    }
                    else
                    {
                        if (wrapper.objs.ContainsKey(fsmVariable.Name))
                            fsmVariable.RawValue = wrapper.objs[fsmVariable.Name];
                    }
                }
            }
        }
...
namespace ES3Types
{
    ...
    [ES3PropertiesAttribute("objs", "arrays", "obj", "array")]
