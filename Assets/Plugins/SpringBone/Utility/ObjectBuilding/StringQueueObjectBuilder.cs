        ...
        public static class ObjectBuilder
        {
            ...
            public static Transform DequeueTransform(this Queue<string> queue, GameObject gameObject)
            {
                ...
                if (parentName.Length > 0)
                {
                    ...
                    newParent = FindObjectsOfType<Transform>(false)
            ...
            public static T[] FindObjectsOfType<T>(bool includeInactice) where T : Component
            {
                Scene curScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene();
                if (!curScene.isLoaded) return null;
                GameObject[] rootObjs = curScene.GetRootGameObjects();
                List<T> retObjs = new List<T>();
                for (int i = 0; i < rootObjs.Length; i++)
                {
                    if (!(includeInactice | rootObjs[i].activeSelf)) continue;
                    Transform[] childs = rootObjs[i].GetComponentsInChildren<Transform>(includeInactice);
                    for (int j = 0; j < childs.Length; j++)
                    {
                        T c = childs[j].GetComponent<T>();
                        if (c) retObjs.Add(c);
                    }
                }
                return retObjs.ToArray();
            }
            public static UnityEngine.Object[] FindObjectsOfType(Type type, bool includeInactice)
            {
                Scene curScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene();
                if (!curScene.isLoaded) return null;
                GameObject[] rootObjs = curScene.GetRootGameObjects();
                List<UnityEngine.Object> retObjs = new List<UnityEngine.Object>();
                for (int i = 0; i < rootObjs.Length; i++)
                {
                    if (!(includeInactice | rootObjs[i].activeSelf)) continue;
                    Transform[] childs = rootObjs[i].GetComponentsInChildren<Transform>(includeInactice);
                    for (int j = 0; j < childs.Length; j++)
                    {
                        UnityEngine.Object c = childs[j].GetComponent(type);
                        if (c) retObjs.Add(c);
                    }
                }
                return retObjs.ToArray();
            }
            public static UnityEngine.Object FindObjectOfType(Type type, bool includeInactice)
            {
                Scene curScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene();
                if (!curScene.isLoaded) return null;
                GameObject[] rootObjs = curScene.GetRootGameObjects();
